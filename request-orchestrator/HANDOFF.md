...
עדכון: כדי לא לחכות לפרטי CCR, שיניתי את ברירת המחדל ל-`ADAPTER=localCli` — **אפשר להריץ
היום, בלי אף סוד**, כל עוד ה-CLI של `claude` מותקן על מכונת המשרד (מה שכבר צריך להיות נכון,
כי כך רצות סשנים מקומיים היום). לכן סעיף 1 הפך לאופציונלי בשלב זה.

1. *(אופציונלי — לשדרוג לענן בעתיד)* **פרטי Claude Code Remote API** — כתובת + טוקן ל-`.env`
   (`CCR_API_URL`, `CCR_API_TOKEN`). `CCR_ENV_ID` כבר ידוע (`env_01Ee5tLoXsNzojCWpfvhmBxc`).
   יש `TODO` אחד ב-`ccrApi.js` על ה-endpoint המדויק של יצירת סשן — צריך לאמת מול התיעוד שלך.
2. **hook בצד `/requests`** — שורת `fetch("http://localhost:4500/approve", …)` בכפתור "אשר"
   (קוד מלא ב-`README.md`). אם קוד `/requests` נמצא ב-GitHub — תן לי שם ה-repo ואני אוסיף
   את זה ישירות, בלי שתצטרך לגעת בקוד בעצמך.
3. **ייצוא הבקשות הקיימות** (JSON/CSV) — כדי שאעבור על הפתוחות, תגובות לברינה/לך, וניסוחים לא-ברורים.
   הסשן בענן לא מגיע ל-localhost:3000. אם יש כבר כפתור ייצוא/CSV במערכת — זה הכי מהיר; אחרת
   גם הדבקה גולמית של הטבלה (copy-paste) מספיקה לי.
