...
python -m src.whatsapp_agent send --to "יוליה" --text "..."   # הודעה אחת לאיש קשר
```

## מה נחשב "ממתין לתשובה"

"ההודעה האחרונה שלהם" זה לא אותו דבר כמו "הם מחכים לי". שיחה שנגמרה ב"תודה"
או "צודק" היא סגורה, ודוח מלא בכאלה הוא דוח שאף אחד לא קורא.

`pending` מפריד לשתי רשימות: שיחות שנראות **פתוחות** (שאלה, בקשה, סימן שאלה),
ומתחתן שיחות שההודעה האחרונה בהן נראית **סגירה**. שום דבר לא מוסתר - רק ממוין,
כי הזיהוי הוא ניחוש והשיפוט שלך. `--all` מציג הכל כרשימה אחת.

`schedule` מציע פגישה רק לשיחות הפתוחות. `--include-closed` כדי לכלול את השאר.
